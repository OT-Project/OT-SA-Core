      ...
      <div class="container-fluid">
        <div class="navbar-header">
          <a class="navbar-brand" href="/">
            <!-- Logo -->
            {% if theme_name == 'otsa-dark' and
            file_exists(["/usr/local/opnsense/www/themes/",theme_name,"/build/images/default-logo-dark.svg"]|join(""))
            %}
            <img class="brand-logo"
              src="{{ cache_safe('/ui/themes/%s/build/images/default-logo-dark.svg' | format(theme_name)) }}"
              height="30" alt="logo" />
            {% elseif
            file_exists(["/usr/local/opnsense/www/themes/",theme_name,"/build/images/default-logo.svg"]|join("")) %}
            <img class="brand-logo"
              src="{{ cache_safe('/ui/themes/%s/build/images/default-logo.svg' | format(theme_name)) }}" height="30"
              alt="logo" />
            {% else %}
            <img class="brand-logo"
              src="{{ cache_safe('/ui/themes/%s/build/images/default-logo.png' | format(theme_name)) }}" height="30"
              alt="logo" />
            {% endif %}

            {% if file_exists(["/usr/local/opnsense/www/themes/",theme_name,"/build/images/icon-logo.svg"]|join("")) %}
            <img class="brand-icon"
              src="{{ cache_safe('/ui/themes/%s/build/images/icon-logo.svg' | format(theme_name)) }}" height="30"
              alt="icon" />
            {% else %}
            <img class="brand-icon"
              src="{{ cache_safe('/ui/themes/%s/build/images/icon-logo.png' | format(theme_name)) }}" height="30"
              alt="icon" />
            {% endif %}
          </a>
        ...
        <button class="toggle-sidebar" data-toggle="tooltip right" title="{{ lang._('Toggle sidebar') }}"
          style="display:none;"><i class="fa fa-chevron-left"></i></button>
        <div class="collapse navbar-collapse">
          <ul class="nav navbar-nav navbar-right">
            <li id="menu_messages">
              <span class="navbar-text navbar-user">{{session_username}}@{{system_hostname}}.{{system_domain}}</span>
            </li>
            <li>
              <span class="navbar-text navbar-status">
                <i id="system_status" data-toggle="tooltip left" title="{{ lang._('Show system status') }}"
                  style="cursor:pointer" class="fa fa-circle text-muted"></i>
              </span>
            </li>
            <li>
              <form class="navbar-form navbar-search-form" role="search">
                <div class="navbar-search-wrapper">
                  ...
                  <input type="text" class="form-control navbar-search-input" tabindex="1" data-provide="typeahead"
                    id="menu_search_box" autocomplete="off" placeholder="{{ lang._('Search...') }}">
                </div>
              </form>
            </li>
          </ul>
